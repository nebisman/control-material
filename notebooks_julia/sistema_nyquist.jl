...
begin
    ...
    #   w180 : frec. de cruce de fase   (∠L = -180°)  → ubica el margen de ganancia
    #   gm  : margen de ganancia (razón, no dB)
    #   wgc : frec. de cruce de ganancia (|L| = 1)   → ubica el margen de fase
    #   ϕₘ  : margen de fase (grados)
...
let
    ...
    scatter!(p1,[ω180], [-GM_dB] ; c=:red, ms=5, label="")
    ...
    p2 = plot(w, phase; xscale=:log10, lw=2, c=cyan, label="",
              ...
              legend=:bottomleft, grid=true)
    plot!(p2, [ωgc, ωgc],[-180,-180+ϕₘ]; c=:orange,lw=2, label="")

    hline!(p2, [-180]; ls=:dash, c=:gray, label="-180°")
    vline!(p2, [ω180]; ls=:dot, c=:red, label="ω cruce fase")
    vline!(p2, [ωgc]; ls=:dot, c=:orange, label="")
    scatter!(p2, [ωgc], [-180 + ϕₘ]; c=:orange, ms=5, label="")   # margen de fase
    scatter!(p2,[ω180], [-180] ; c=:red, ms=5, label="")
    annotate!(p2, ωgc, -180 + ϕₘ,
              text("  ϕₘ=$(round(ϕₘ, digits=1))°", 8, :left, :orange))
    ...
    θ = deg2rad(-180 + ϕₘ)
    scatter!(p3, [cos(θ)], [sin(θ)]; c=:orange, ms=6, label="ϕₘ =$(round(ϕₘ,digits=1))°")
...
| **Margen de fase** (ϕₘ) | $(round(ϕₘ, digits=2)) ° | ωgc = $(round(ωgc, digits=3)) rad/s |
